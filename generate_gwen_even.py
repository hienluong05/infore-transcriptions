#!/usr/bin/env python3
"""Generate even-numbered physical lines of a resumable Gwen-TTS dataset.
...
def load_utterances(path: Path, max_chars: int, parity: int | None = None) -> list[dict[str, Any]]:
    items = []
    for line_no, raw in enumerate(path.read_text(encoding='utf-8-sig').splitlines(), 1):
        # Count every physical line, including empty lines. 1=odd, 0=even.
        if parity is not None and line_no % 2 != parity:
            continue
        text = clean_text(raw)
        if not text:
            continue
...
def write_manifests(output: Path, items: list[dict], sr: int, suffix: str = ""):
    """Shard outputs have unique names; combined outputs are written only via --merge-only."""
    ...
    for name, lines in files.items():
        filename = f'{Path(name).stem}_{suffix}{Path(name).suffix}' if suffix else name
...
def parse_args():
    ...
    p.add_argument('--merge-only', action='store_true',
                   help='After BOTH workers finish, rebuild combined metadata.csv/filelist.txt/manifest.jsonl/details.jsonl without GPU')
    return p.parse_args()
...
def main():
    ...
    all_items = load_utterances(args.text_file, args.max_chars)
    items = [item for item in all_items if item['line'] % 2 == PARITY]
    if args.limit and not args.merge_only:
    ...
    LOG.info('Shard=%s | Source: %s | utterances: %d | ref: %s (%.1fs) | output: %s',
             SHARD, args.text_file, len(items), args.ref_audio, ref_info.duration, out)
    if args.merge_only:
        count = write_manifests(out, all_items, args.sample_rate)
        LOG.info('Merged %d valid WAV+record pairs into combined manifests (%d expected).', count, len(all_items))
        return
    ...
    completed = failed = 0
    failures = out / f'failures_{SHARD}.jsonl'
    try:
        for index, item in enumerate(items, 1):
            ...
            if index % 20 == 0:
                write_manifests(out, items, args.sample_rate, suffix=SHARD)
    finally:
        count = write_manifests(out, items, args.sample_rate, suffix=SHARD)
        LOG.info('Saved %d valid samples to manifests | completed=%d failed=%d',
                 count, completed, failed)
    LOG.info('Shard manifests: %s | Run --merge-only AFTER both workers finish for combined manifests.', out)
